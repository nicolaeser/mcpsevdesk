...
import { defaultClientFactory, probeSevdeskToken, type UpstreamTokenCheck } from "../sevdesk/client.js";
import {
  HEALTH_PATH,
  MCP_PATH,
  type HttpBindConfig,
...
export interface HttpAppOptions {
  ...
  readonly verifyApiToken?: false | UpstreamTokenCheck;
}
...
export function createHttpApp(options: HttpAppOptions = {}): express.Express {
  ...
  const verifyApiToken =
    options.verifyApiToken === false || options.verifyApiToken === undefined
      ? undefined
      : options.verifyApiToken;
  mountOAuth(app, oauth, config, verifyApiToken);
  ...
  app.all(MCP_PATH, async (req: Request, res: Response) => {
    ...
    if (!credentials.ok) {
      process.stderr.write(`auth.mcp outcome=${credentials.code === "missing" ? "missing" : "invalid"}\n`);
...
export async function runHttp(options: HttpAppOptions = {}): Promise<Server> {
  ...
  const app = createHttpApp({
    ...
    env,
    verifyApiToken:
      options.verifyApiToken === false
        ? false
        : (options.verifyApiToken ??
          ((token) => probeSevdeskToken(token, options.createClient ?? defaultClientFactory)))
  });
...
function mcpOptions(
  createClient: SevdeskClientFactory,
  token: string,
