...
export const WWW_AUTHENTICATE = 'Bearer realm="mcpsevdesk", scope="mcp:tools"';
export const WWW_AUTHENTICATE_INVALID =
  'Bearer realm="mcpsevdesk", scope="mcp:tools", error="invalid_token"';
...
export function readRuntimeConfig(env: EnvLike = process.env): RuntimeConfig {
  const apiToken = nonEmpty(env.SEVDESK_API_TOKEN);
  const baseURL = nonEmpty(env.SEVDESK_BASE_URL);
  const host = nonEmpty(env.MCPSEVDESK_HOST) ?? nonEmpty(env.MCP_HOST) ?? DEFAULT_HTTP_HOST;
  const port = parsePort(env.MCPSEVDESK_PORT ?? env.MCP_PORT, DEFAULT_HTTP_PORT);
  ...
  return {
    ...
    allowInsecureLocalhost: baseURL !== undefined && isLoopbackHttp(baseURL),
    http,
...
function readAuthConfig(env: EnvLike, http: HttpBindConfig): AuthConfig {
  ...
  const publicUrl = normalizePublicOrigin(nonEmpty(env.MCPSEVDESK_PUBLIC_URL) ?? nonEmpty(env.MCP_PUBLIC_URL), "MCP_PUBLIC_URL");
...
function parsePort(value: string | undefined, fallback: number): number {
  ...
  if (!Number.isInteger(port) || port < 0 || port > 65535) {
    throw new Error(`MCPSEVDESK_PORT must be an integer 0–65535, got ${value}`);
  }
  return port;
}
