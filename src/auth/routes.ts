...
import type { UpstreamTokenCheck } from "../sevdesk/client.js";
...
export function mountOAuth(
  ...
  verifyApiToken?: UpstreamTokenCheck
): void {
  ...
  const authorize = (req: Request, res: Response, next: NextFunction) => {
    void handleAuthorize(req, res, service, config, req.method === "POST", verifyApiToken).catch(next);
  };
  ...
  app.post("/token", (req, res) => {
    ...
    const grant = body.grant_type === "refresh_token" ? "refresh" : "code";
    try {
      if (body.grant_type === "authorization_code") {
        ...
        process.stderr.write("auth.token grant=code ok\n");
      ...
      if (body.grant_type === "refresh_token") {
        ...
        process.stderr.write("auth.token grant=refresh ok\n");
        res.status(200).json(tokens);
        return;
      }
      res.status(400).json({ error: "unsupported_grant_type" });
    } catch (error) {
      process.stderr.write(`auth.token grant=${grant} fail\n`);
...
async function handleAuthorize(
  ...
  config: RuntimeConfig,
  isPost: boolean,
  verifyApiToken: UpstreamTokenCheck | undefined
): Promise<void> {
  res.setHeader("Cache-Control", "no-store");
  res.setHeader(
  ...
  const bind: CsrfBind = { clientId, redirectUri, codeChallenge: params.code_challenge };
  ...
  const consentValue = String(params.consent ?? "");
  const isConsent = isPost && consentValue === "1";
  const isCancel = isPost && consentValue === "0";
  if (!isConsent && !isCancel) {
  ...
  const csrfBody = typeof params.csrf === "string" ? params.csrf : undefined;
  const verified = service.verifyCsrf(csrfBody, bind);
  if (!verified.ok) {
    if (verified.reason === "replay" && csrfBody !== undefined) {
      const existing = service.unusedCodeForCsrf(csrfBody);
    ...
    showConsent(401, CONSENT_SESSION_EXPIRED, service.issueCsrf(bind));
    return;
  }
  if (isCancel) {
  ...
  if (typeof params.website === "string" && params.website.trim().length > 0) {
    showConsent(401, CONSENT_GENERIC, csrfBody ?? service.issueCsrf(bind));
    return;
  }
  ...
  if (!collected.ok) {
    showConsent(401, collected.error, csrfBody ?? service.issueCsrf(bind));
    return;
  }
  const upstream = collected.bag.secrets.apiToken?.trim() ?? "";
  if (looksLikeMcpToken(upstream)) {
    showConsent(401, "Enter the SevDesk API token, not the MCP session token.", csrfBody ?? service.issueCsrf(bind));
    return;
  }
  if (!service.consumeCsrf(csrfBody ?? "")) {
    showConsent(401, CONSENT_SESSION_EXPIRED, service.issueCsrf(bind));
    return;
  }
  if (verifyApiToken !== undefined && upstream.length > 0) {
    const checked = await verifyApiToken(upstream);
    if (!checked.ok) {
      showConsent(401, checked.error, service.issueCsrf(bind));
      return;
    }
  }
...
function limited(req: Request, bucket: string, max: number, windowMs: number): boolean {
