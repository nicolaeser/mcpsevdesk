...
import { filterRedirectUris, isAllowedRedirect, logRejectedRedirect, redirectUriMatches } from "./redirects.js";
...
export class McpOAuthService {
  ...
  public issueCsrf(bind: CsrfBind): string {
    ...
    const mac = signCsrf(this.secret(), `${CSRF_PREFIX}.${payload}`);
    const token = `${CSRF_PREFIX}.${payload}.${mac}`;
  ...
  public verifyCsrf(
    ...
    bind: CsrfBind
  ): { ok: true } | { ok: false; reason: CsrfFailReason } {
    if (token === undefined || token.length === 0) return { ok: false, reason: "hmac" };
  ...
  private parseCsrf(token: string): CsrfPayload | undefined {
    ...
    if (!verifyCsrfMac(this.secret(), `${CSRF_PREFIX}.${payload}`, mac)) return undefined;
    try {
      const parsed = JSON.parse(Buffer.from(payload, "base64url").toString("utf8")) as CsrfPayload;
  ...
  public collectBag(body: Record<string, string | undefined>): ReturnType<typeof collectLoginBag> {
    if (!this.requirePassword) {
      const submitted = normalizeSecretToken(body.apiToken?.trim() ?? "");
      if (submitted.length === 0) return { ok: false, error: "API token is required." };
    }
    return collectLoginBag(this.fields, body, this.env);
  }

  public allowRedirect(uri: string): boolean {
    const allowed = isAllowedRedirect(uri);
