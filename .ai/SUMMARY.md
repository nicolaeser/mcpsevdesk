...
This package is the sevdesk MCP server. Upstream work goes through `npmsevdesk`.
...
- [architecture.md](knowledge/architecture.md) — Maps stdio vs HTTP, the `npmsevdesk` client, tool
  catalog, and process entry points and must be loaded when choosing where a change belongs.
