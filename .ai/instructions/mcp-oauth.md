...
- HTTP `/mcp` must reject any Bearer that is not an `mcp1.` access token issued by this process.
  Sevdesk API tokens must not authenticate the connector.
- Stdio must keep using `SEVDESK_API_TOKEN` / `SEVDESK_BASE_URL` and must not speak OAuth.
...
- `publicOrigin` must use `MCPSEVDESK_PUBLIC_URL` or `MCP_PUBLIC_URL` when set and must not trust
  `X-Forwarded-Host` as the issuer when it is unset. A configured public URL must be stored as
  `URL.origin` (path stripped).
- Authorization codes must lock the CSRF nonce (`consumeCsrf` returns false on reuse) before any
  await, including the SevDesk token probe.
- Token endpoint auth methods advertised to connectors must be `none` (public PKCE).
...
- `/mcp` Host headers must match loopback or the configured public hostname. Binding `0.0.0.0`
  must not disable the allowlist. GET `/authorize` is also Host-checked.
